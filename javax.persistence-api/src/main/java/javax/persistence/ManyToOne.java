 ...
 * リレーションシップが双方向である場合、被所有側の<code>OneToMany</code>エンティティでは<code>mappedBy</code>要素を使用して、
 * 所有側のエンティティのリレーションシップを示すエンティティのリレーションシップのフィールドまたはプロパティを指定する必要があります。
 * 
 * <p> <code>ManyToOne</code>アノテーションは組み込みクラス({@link Embeddable}の付いたクラス)からエンティティクラスへのリレーションシップを指定するために組み込みクラス内で使用できます。
 * リレーションシップが双方向である場合、被所有側の<code>OneToMany</code>エンティティでは<code>OneToMany</code>アノテーションの<code>mappedBy</code>要素を使用して
