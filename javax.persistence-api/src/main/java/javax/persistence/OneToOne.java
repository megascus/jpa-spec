 ...
 * リレーションシップが双方向である場合、被所有側のエンティティは<code>OneToOne</code>アノテーションの<code>mappedBy</code>要素を使用して、
 * 所有側のエンティティのリレーションシップを示すフィールドまたはプロパティを指定する必要があります。
 * 
 * <p> <code>OneToOne</code>アノテーションは組み込みクラス({@link Embeddable}の付いたクラス)からエンティティクラスへのリレーションシップを指定するために組み込みクラス内で使用できます。
 * リレーションシップが双方向であり、組み込みクラスを含むエンティティがリレーションシップの所有側のエンティティである場合、
 * 被所有側のエンティティでは、組み込みクラスのリレーションシップを示すフィールドまたはプロパティを指定するために<code>OneToOne</code>アノテーションの<code>mappedBy</code>要素を使用する必要があります。
...
public @interface OneToOne {
     ...
     * 
     * もしfalseである場合、このリレーションシップは常にnullでない必要があります。
     */
