 ...
 * 被所有側のエンティティでは、組み込みクラスの関係を示すフィールドまたはプロパティを指定するために<code>OneToOne</code>アノテーションの<code>mappedBy</code>要素を使用する必要があります。
...
public @interface OneToOne {
     ...
     * 
     * EAGER戦略は永続性プロバイダの実行時に関連エンティティを即座に取得するべきだとする要件です。
     * LAZY戦略は持続性プロバイダの実行時のヒントです。
     * 
