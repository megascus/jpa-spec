 ...
 * 
 * <code>name</code>要素が指定されていない場合、デフォルトでは次のものが連結されます。：参照元のリレーションシップのフィールドまたはプロパティの名前 + "_" + "キー"
 * 
 * <pre>
 *    Example:
 *
...
public @interface MapKeyColumn {
	 ...
	 * 要素コレクションのためのマップキーの場合、マップキーのカラムはマップの値のコレクションテーブルに存在します。
     * ManyToManyのエンティティのリレーションシップや結合テーブルを使用したOneToManyのエンティティのリレーションシップのためのマップキーの場合、マップキーのカラムは結合テーブルに存在します。
     * 外部キーマッピング戦略を使用したOneToManyのエンティティのリレーションシップのためのマップキーの場合、マップキーのカラムは値がマッピングされたテーブルに存在します。
     * <p> デフォルトでは次のものが連結されます。：参照元のリレーションシップのフィールドまたはプロパティの名前 + "_" + "キー"
	 */
	String name() default "";
	 ...
	 * 結合テーブルを使用してマップキーがOneToManyまたはManyToManyエンティティのリレーションシップにある場合はマップの結合テーブルの名前。
	 * 外部キーマッピング戦略を使用してマップキーがOneToManyエンティティのリレーションシップ用である場合はマップの値のエンティティのプライマリテーブルの名前。
     */
	String table() default "";

	/**
	 * (オプション) カラムの長さ。(文字列値のカラムが使用されている場合のみ適用されます。)
	 */
