 ...
 * ただし、双方向多対1や1対多の関連付け、単方向多対1のリレーションシップおよび1対1の関連付け(双方向および単方向の両方)をマッピングするためでも使用できます。
 * 
 * <p>リレーションシップの所有側で組み込みクラスとのリレーションシップをマッピングする際に結合テーブルを使用した場合、
 * 組み込みクラスではなく組み込みクラスを保持するエンティティがリレーションシップの所有者とみなされます。
 * 
...
public @interface JoinTable {
    ...
    /**
     * (オプション) 関連を所有するエンティティのプライマリテーブルを参照する結合テーブルの外部キーのカラム。(つまり、関連の所有側)
     *
    ...
    /** 
     * (オプション) 関連を所有しないエンティティのプライマリテーブルを参照する結合テーブルの外部キーのカラム。(つまり、関連の所有の逆側)
     *
     * <p> {@link JoinColumn}と同じデフォルト値を使用します。
     */
    ...
    /**
     * (オプション) テーブルの生成が有効な場合に<code>inverseJoinColumns</code>要素に関係するカラムのための外部キー制約の生成を指定または制御するために使用されます。
     * 
